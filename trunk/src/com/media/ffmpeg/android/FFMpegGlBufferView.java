...
public class FFMpegGlBufferView extends GLSurfaceView {
	private native void render(int ptr);

	private native void init(int ptr, int w, int h);

	private MyRenderer render;
	private static String TAG = "FFMpegGlBufferView";
	private static final boolean DEBUG = false;

	public FFMpegGlBufferView(Context context) {
		super(context);
		init(false, 0, 0);
	}

	public FFMpegGlBufferView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(false, 0, 0);
	}

	public FFMpegGlBufferView(Context context, boolean translucent, int depth,
			int stencil) {
		super(context);
	...
	public void init(boolean translucent, int depth, int stencil) {
		 ...
		 * (new Thread() {
		 * 
		 * @Override public void run() { native_start(); } }).start();
		 */
		Log.i(TAG, "Create GlBufferView");
		setEGLContextFactory(new ContextFactory());
		Log.i(TAG, "Create ContextFactory");
		setEGLConfigChooser(translucent ? new ConfigChooser(8, 8, 8, 8, depth,
				stencil) : new ConfigChooser(5, 6, 5, 0, depth, stencil));
		Log.i(TAG, "Create MyRenderer");
		setRenderer((render = new MyRenderer()));
	 ...
	 * int height) { Log.w(TAG, "Video preview surface changed");
	 * LinphoneManager.getLc().setVideoWindow(this); }
	 * 
	 * public void surfaceCreated(SurfaceHolder holder) { Log.w(TAG,
	 * "Video preview surface created"); }
	 * 
	 * public void surfaceDestroyed(SurfaceHolder holder) { Log.d(TAG,
	 * "Video preview surface destroyed");
	...
	public void setOpenGLESDisplay(int ptr) {
		render.setOpenGLESDisplay(ptr);
	}

	public void requestRender2() {
		requestRender();
	}
	...
	public void onPause() {
		...
		super.onPause();
		System.out.println("NovakGlBufferView.onPause()");
	}
	...
	public void onResume() {
		// TODO Auto-generated method stub
		super.onResume();
		System.out.println("NovakGlBufferView.onResume()");
		//	
	}
